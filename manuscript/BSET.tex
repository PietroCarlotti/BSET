...
\label{eq:correlation_matrix}
  \Omega = \begin{bmatrix}
    ...
    \end{bmatrix}.
\end{align}
It is easy to verify that the green entries in the correlation matrix from Equation~\eqref{eq:correlation_matrix} are identifiable from the data, since they involve correlations between potential outcomes that are observed together in the data. In contrast, the remaining entries are not identifiable from the data, since they involve correlations between potential outcomes that are never jointly observed for any unit. Moreover, as the general formula for \(\theta\) in Web Appendix~\ref{app:derivation_theta_delta} shows, derived without assuming conditional orthogonality and homoscedasticity, the red entries directly affect the computation of \(\theta\), while the orange entries do not.
...
Under standard regularity conditions - positive prior mass to every KL neighborhood of the true parameters and model identifiability - Schwartz's theorem \citep{schwartz1965bayes} guarantees that the posterior concentrates around the true parameter value (see Web Appendix~\ref{app:posterior_consistency} for the formal statement and definition of regularity). While our model satisfies the first condition, it lacks identifiability, meaning posterior concentration around the true \(\theta\) is not guaranteed. Instead, the posterior follows the behavior characterized by \cite{gustafson2010bayesian}:
\begin{theorem}
