...
\label{eq:correlation_matrix}
  \Omega = \begin{bmatrix}
    ...
    \end{bmatrix}.
\end{align}
It is easy to verify that the green entries in the correlation matrix from Equation~\eqref{eq:correlation_matrix} are identifiable from the data, since they involve correlations between potential outcomes that are observed together in the data. In contrast, the remaining entries are not identifiable from the data, since they involve correlations between potential outcomes that are never observed together for any unit. Moreover, as the general formula for \(\theta\) in Appendix~\ref{app:derivation_theta_delta} shows, derived without making the two assumptions of conditional orthogonality and homoscedasticity, the red entries directly affect the computation of \(\theta\), while the orange entries do not.
...
Under standard regularity conditions - namely, positive prior mass to every KL neighborhood of the true parameters and model identifiability - Schwartz's theorem \citep{schwartz1965bayes} guarantees that the posterior concentrates around the true parameter value (see Appendix~\ref{app:posterior_consistency} for the formal statement and definition of regularity). While our model satisfies the first condition, it lacks identifiability, meaning posterior concentration around the true $\theta$ is not guaranteed. Instead, the posterior follows the behavior characterized by \cite{gustafson2010bayesian}:
\begin{theorem}
...
The resulting Bayes factor $BF_n$ is derived and given in closed form in Appendix~\ref{app:BF_derivation}. Then, as usual, we would like to ensure a type I error probability of $\alpha$:
\begin{equation}
    \mathbb{P} (BF_{n} > BF_{n, \alpha} \mid H_0) = \alpha.
\end{equation}
Since $n \, \widehat{V}_{S}$ takes values in $\{0, \ldots, n\}$, $BF_{n}$ has at most $n + 1$ distinct values, so $BF_{n, \alpha}$ can be easily obtained as the $(1 - \alpha)$-quantile of the distribution of $BF_{n}$ under $H_0$, which is given in closed form in Appendix~\ref{app:BF_derivation}. Then, given $BF_{n, \alpha}$, we define $v_{S}$ as the value such that
\begin{equation}
