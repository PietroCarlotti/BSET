...
\begin{align}
    ...
    \end{split}
\end{align}
for some constant \(\Delta > 0\). One can show (see Appendix~\ref{app:binary_covariate_derivation}) that the discrepancy between \(\theta\) and \(\delta\) is driven by \(\Delta\) and the difference between means of \(Y\) and \(S\) for control units across different \(X\) groups. Specifically,
\begin{align}
...
\end{align}
One can show (see Appendix~\ref{app:continuous_covariate_derivation}) that \(\delta\) and \(\theta\) are given by:
\begin{align}
...
Then, given the values of \(m\), \(s\), \(\Delta\), if \(m \neq 0\) and \(\Delta \neq 0\), then \( |\theta - \delta| = |\delta| \neq 0\) and its value is entirely driven by the magnitude of \(\beta_{Y_{0}}\) and \(\beta_{S_{0}}\). One can further show (see Appendix~\ref{app:continuous_covariate_derivation}) that \(\sup_{\beta_{Y_{0}}, \beta_{S_{0}}, \Delta} \{ |\delta| \} = |\Phi(m/s) - 0.5|\), which can be arbitrarily close to \(0.5\) if \(|m/s|\) is large enough (i.e., the covariate is highly concentrated around a non-zero value). This means that in this setting, even though \(S\) is a perfect surrogate for \(Y\), \(\delta\) can be substantially different from \(0\) and its value depends arbitrarily on the distribution of the covariate. In contrast, \(\theta\) correctly captures the surrogate validity by being equal to \(0\).
