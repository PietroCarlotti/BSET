...
Our analysis focuses on a small subgroup of high-risk participants: those in the secondary-intervention cohort (with baseline mild-to-moderate retinopathy) with a body mass index over 25.0 and a history of smoking. This yields a sample of 47 participants. Figure~\ref{fig:dcct} shows the distribution of the primary outcome and the surrogate by treatment arm. Figure~\ref{fig:dcct_scatter_cdf} illustrates the relationship between the surrogate and primary outcome, along with the empirical CDFs, whose horizontal gap directly corresponds to the rank-based quantities \(U_Y\) and \(U_S\).
