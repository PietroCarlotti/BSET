...
\begin{align}
    Y_{1i} - Y_{0i} \mid X_{i} = x \overset{\text{\footnotesize ind.}}{\sim} \mathcal{N} \left( \frac{ \mu^{x}_{1} - \mu^{x}_{3}}{\sqrt{\Sigma^{x}_{11} + \Sigma^{x}_{33} - 2 \Sigma^{x}_{13}}} \right), \quad i = 1, \ldots, n, \; \forall \; x \in \mathcal{X}.
\end{align}
...
\begin{align}
    \begin{split}
        ...
        & = \Prob \left( \frac{Y_{1i} - Y_{0i} - \mu^{x}_{1} + \mu^{x}_{3}}{\sqrt{\Sigma^{x}_{11} + \Sigma^{x}_{33} - 2 \Sigma^{x}_{13}}} > \frac{- \mu^{x}_{1} + \mu^{x}_{3}}{\sqrt{\Sigma^{x}_{11} + \Sigma^{x}_{33} - 2 \Sigma^{x}_{13}}} \mid X_{i} = x \right) \\
        & = \Prob \left( Z > \frac{- \mu^{x}_{1} + \mu^{x}_{3}}{\sqrt{\Sigma^{x}_{11} + \Sigma^{x}_{33} - 2 \Sigma^{x}_{13}}} \right) \\
        & = 1 - \Phi \left( \frac{- \mu^{x}_{1} + \mu^{x}_{3}}{\sqrt{\Sigma^{x}_{11} + \Sigma^{x}_{33} - 2 \Sigma^{x}_{13}}} \right) \\
        & = \Phi \left( \frac{ \mu^{x}_{1} - \mu^{x}_{3}}{\sqrt{\Sigma^{x}_{11} + \Sigma^{x}_{33} - 2 \Sigma^{x}_{13}}} \right), \; \forall \; x \in \mathcal{X},
    \end{split}
\end{align}
...
\begin{align}
    \begin{split}
        ...
        & = \int_{\mathcal{X}} \Phi \left( \frac{ \mu^{x}_{1} - \mu^{x}_{3}}{\sqrt{\Sigma^{x}_{11} + \Sigma^{x}_{33} - 2 \Sigma^{x}_{13}}} \right) \, f (x) \, \text{d}x.
    \end{split}
\end{align}
%\vspace{-1mm}
...
\label{eq:decomposition_theta}
    \begin{split}
        ...
        & = \int_{\mathcal{X}} \left( \Phi \left( \frac{ \mu^{x}_{1} - \mu^{x}_{3}}{\sqrt{\Sigma^{x}_{11} + \Sigma^{x}_{33} - 2 \Sigma^{x}_{13}}} \right) - \Phi \left( \frac{ \mu^{x}_{2} - \mu^{x}_{4}}{\sqrt{\Sigma^{x}_{22} + \Sigma^{x}_{44} - 2 \Sigma^{x}_{24}}} \right) \right) \, f (x) \, \text{d}x,
    \end{split}
\end{align}
...
\begin{align}
    Y_{1i} - Y_{0j} \mid X_{i} = x_{i}, X_{j} = x_{j} \sim \mathcal{N} \left( \frac{\mu^{x_{i}}_{1} - \mu^{x_{j}}_{3}}{\sqrt{\Sigma^{x_{i}}_{11} + \Sigma^{x_{j}}_{33}}} \right), \quad \forall \; x_{i}, x_{j} \in \mathcal{X}, \; i,j = 1, \ldots, n,
\end{align}
then we have that
\begin{align}
    \begin{split}
        U^{x_{i} x_{j}}_{Y} & = \Prob \left( Y_{1i} > Y_{0j} \mid X_{i} = x_{i}, X_{j} = x_{j} \right) \\
        & = \Prob \left( \frac{Y_{1i} - Y_{0j} - \mu^{x_{i}}_{1} + \mu^{x_{j}}_{3}}{\sqrt{\Sigma^{x_{i}}_{11} + \Sigma^{x_{j}}_{33}}} > \frac{- \mu^{x_{i}}_{1} + \mu^{x_{j}}_{3}}{\sqrt{\Sigma^{x_{i}}_{11} + \Sigma^{x_{j}}_{33}}} \mid X_{i} = x_{i}, X_{j} = x_{j} \right) \\
        & = \Prob \left( Z > \frac{- \mu^{x_{i}}_{1} + \mu^{x_{j}}_{3}}{\sqrt{\Sigma^{x_{i}}_{11} + \Sigma^{x_{j}}_{33}}} \right) \\
        & = 1 - \Phi \left( \frac{- \mu^{x_{i}}_{1} + \mu^{x_{j}}_{3}}{\sqrt{\Sigma^{x_{i}}_{11} + \Sigma^{x_{j}}_{33}}} \right) \\
        & = \Phi \left( \frac{\mu^{x_{i}}_{1} - \mu^{x_{j}}_{3}}{\sqrt{\Sigma^{x_{i}}_{11} + \Sigma^{x_{j}}_{33}}} \right) , \quad \forall \; x_{i}, x_{j} \in \mathcal{X}, \; i,j = 1, \ldots, n,
    \end{split}
\end{align}
...
\begin{align}
    \begin{split}
        ...
        & = \int_{\mathcal{X}} \int_{\mathcal{X}} \Phi \left( \frac{\mu^{x_{i}}_{1} - \mu^{x_{j}}_{3}}{\sqrt{\Sigma^{x_{i}}_{11} + \Sigma^{x_{j}}_{33}}} \right) \, f (x_{i}) \, f (x_{j}) \, \text{d}x_{i} \, \text{d}x_{j}.
    \end{split}
\end{align}
...
\begin{align}
    \begin{split}
        ...
        & = \int_{\mathcal{X}} \int_{\mathcal{X}} \left( \Phi \left( \frac{\mu^{x_{i}}_{1} - \mu^{x_{j}}_{3}}{\sqrt{\Sigma^{x_{i}}_{11} + \Sigma^{x_{j}}_{33}}} \right) - \Phi \left( \frac{\mu^{x_{i}}_{2} - \mu^{x_{j}}_{4}}{\sqrt{\Sigma^{x_{i}}_{22} + \Sigma^{x_{j}}_{44}}} \right) \right) \, f (x_{i}) \, f (x_{j}) \, \text{d}x_{i} \, \text{d}x_{j},
    \end{split}
\end{align}
where \( \delta^{x_{i} x_{j}} = U^{x_{i} x_{j}}_{Y} - U^{x_{i} x_{j}}_{S} \; \forall \; x_{i}, x_{j} \in \mathcal{X}, \; i,j = 1, \ldots, n\).

While it may appear that the formulas for \(\delta\) and \(\theta\) are similar, they are not exactly the same. If we are willing to assume that \( \Sigma^{x}_{13} = \Sigma^{x}_{24} = 0 \; \forall \; x \in \mathcal{X} \) then it follows that
\begin{align}
  V^{x}_{Y} = U^{x x}_{Y} \text{ and } V^{x}_{S} = U^{x x}_{S} \; \forall \; x \in \mathcal{X}.
\end{align}
Moreover, if we define the function
\begin{align}
    g(x_{i}, x_{j}) = \Phi \left( \frac{\mu^{x_{i}}_{1} - \mu^{x_{j}}_{3}}{\sqrt{\Sigma^{x_{i}}_{11} + \Sigma^{x_{j}}_{33}}} \right) - \Phi \left( \frac{\mu^{x_{i}}_{2} - \mu^{x_{j}}_{4}}{\sqrt{\Sigma^{x_{i}}_{22} + \Sigma^{x_{j}}_{44}}} \right),
\end{align}
...
\begin{align}
    \begin{split}
        ...
        \delta^{01} & = \Phi(\Delta + \mu^{0}_{3} - \mu^{1}_{3}) - \Phi(\Delta + \mu^{0}_{4} - \mu^{1}_{4}) \\
        \delta^{10} & = \Phi(\Delta + \mu^{1}_{3} - \mu^{0}_{3}) - \Phi(\Delta + \mu^{1}_{4} - \mu^{0}_{4})
    \end{split}
\end{align}
...
\end{itemize}
Hence, \(|\theta - \delta| = \frac{1}{4} | 1 - 2 \Phi (\Delta)| \approx \frac{1}{4}\) for \(\Delta\) large enough (e.g., \(\Delta > 3\)) and, in particular, \(\theta = 0\) while \(\delta \approx 0.25\). This means that in this setting, even though \(S\) is a perfect surrogate for \(Y\), \(\delta\) is substantially different from \(0\), while \(\theta\) correctly captures the surrogate validity. 
...
\subsection{Identifiability of \(\theta\) and Interpretation of its Posterior}
...
We evaluated the performance of our proposed Bayesian imputation-based method against the frequentist rank-based approach of Parast et al.\ \citep{parast2024rank} in terms of test coverage and power via a simulation study in five settings.
...
\textcolor{magenta}{[From Layla: Please take a look at this DCCT paper and familiarize yourself a bit with this study. Please also find and add references for each place where I have REF, and fill in any XX text. And of course, change/edit/add as much as you would like.]}
We illustrate our proposed method using  data from the Diabetes Control and Complications Trial (DCCT) which was a multicenter randomized clinical trial designed to compare outcomes under an intensive treatment regimen versus the standard of care, among individuals with Type 1 diabetes \citep{diabetes1993effect}. For our analysis, the primary outcome of interest was change in hemoglobin A1c (HbA1c) from baseline to 4 years post-randomization. HbA1c is measured via the blood and reflects glycemic control. Our surrogate marker of interest is change in albumin excretion rate (AER) from baseline to 4 years post-randomization. AER is measured from urine samples and captures early renal changes associated with diabetes. For both HbA1c and AER, lower values are better and thus, change was defined as the measurement at baseline minus the measurement at 4 years, such that positive numbers indicate a beneficial change. From a practical standpoint, AER may offer advantages as a surrogate for HbA1c in certain settings: urine-based measurements can be collected non-invasively, may be more feasible for frequent or home-based monitoring, and directly reflect physiological processes influenced by glycemic exposure (REF?). 

Our analysis is focused on a subgroup of high-risk participants, specifically male smokers with a body mass index over 25.0 because XXXX (ref?). Our analytic sample was XXX participants. 
We applied the method of \citet{parast2024rank} and our proposed method, both with a specified power of XX used to determine the corresponding thresholds. Using the method of \citet{parast2024rank}, $\widehat{U}_Y= XX$, $\widehat{U}_S= XX$, $\widehat{\delta}= XX$ and the surrogate was not identified as valid. Using our proposed method, $\widehat{V}_Y= XX$, $\widehat{V}_S= XX$, $\widehat{\theta}= XX$ and the surrogate was XXX. Given that a major advantage of our approach is the ability to incorporate covariates, we illustrate the resulting inference when covariates capturing patient-level heterogeneity (e.g., age, XXX) are incorporated into the procedure. With these covariates, $\widehat{V}_Y = \text{XX}$, $\widehat{V}_S = \text{XX}$, and $\widehat{\theta} = \text{XX}$. While the true values are unknown, this application highlights the potential utility of our proposed approach and, in particular, demonstrates improved characterization of surrogacy and increased efficiency compared to the existing approach.


\section{Discussion}\label{sec:discussion}

We proposed a Bayesian imputation framework for surrogate evaluation in randomized clinical trials. Our method overcomes limitations of the frequentist rank-based approach, including lack of causal interpretation, inability to use baseline covariates, and low power, particularly in  in covariate-dependent settings. By imputing unobserved potential outcomes, it enables flexible treatment-control comparisons and inference on surrogate-primary associations. Incorporating baseline covariates improves applicability to complex scenarios, as shown in simulations where our method achieved perfect power versus \(0.075\) for the existing approach. \textcolor{magenta}{Our proposed methods are implemented in the R package \texttt{XXX} available at \url{XXX}, which also contains all reproducible code.}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.8\textwidth]{Plots/discrepancy.png}
