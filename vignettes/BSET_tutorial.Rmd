...
```{r quick-start-no-X-show, echo=FALSE, fig.width=8, fig.height=4.5, fig.align='center', fig.cap="Example output from BSET_no_X: posterior distribution of $\\theta$."}
...
```{r quick-start-X-show, echo=FALSE, fig.width=8, fig.height=4.5, fig.align='center', fig.cap="Example output from BSET_X: posterior distribution of $\\theta$ adjusted for a baseline covariate."}
readRDS("BSET_X_results.rds")$theta_posterior_plot
```

Both functions return the posterior distribution of \(\theta\), the discrepancy between the treatment effects on \(Y\) and \(S\). The blue vertical line marks the upper bound of the 95% credible interval, and the green vertical line marks the validation threshold \(\eta\). If the blue line falls below the green line, there is evidence that the surrogate is valid.
